...
<head>
    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .modal { background: var(--surface); border-radius: var(--radius); width: 100%; max-width: 520px; max-height: 90vh; overflow-y: auto; box-shadow: 0 24px 64px rgba(0,0,0,0.15); animation: mIn .2s ease; }
        ...
        .detail-info-val { font-size: 13px; font-weight: 600; color: var(--text); margin-top: 3px; }
    ...
    <div class="content">
        ...
        <div class="summary-grid">
            ...
            <div class="sum-card">
                ...
                <div>
                    <?php $grandTotal = array_sum(array_column($transaksi, 'total')); ?>
                    <div class="sum-val" style="color:var(--orange-dark);font-size:16px">Rp <?= number_format($grandTotal, 0, ',', '.') ?></div>
            ...
            <div class="sum-card">
                ...
                <div>
                    <?php
                    $today = date('Y-m-d');
                    $todayCount = count(array_filter($transaksi, fn($t) => $t['tanggal'] === $today));
                    ?>
                    <div class="sum-val" style="color:var(--blue)"><?= $todayCount ?></div>
        ...
        <div class="toolbar">
            <div class="search-wrap">
                ...
                <input type="text" class="form-control" id="searchInput" placeholder="Cari kode transaksi..." oninput="filterTrx()">
            </div>
            <a href="/transaksi/create" class="btn btn-orange"><i class="fas fa-plus"></i> Transaksi Baru</a>
        </div>
            ...
            <div class="tw">
                <table id="trxTable">
                    ...
                    <tbody>
                        ...
                        <tr data-kode="<?= strtolower(esc($trx['kode'])) ?>">
                            <td style="color:var(--muted);font-size:12px"><?= $no++ ?></td>
                            <td>
                                <span class="badge b-blue"><?= esc($trx['kode']) ?></span>
                            </td>
                            <td>
                                <div style="font-weight:600"><?= date('d M Y', strtotime($trx['tanggal'])) ?></div>
                                <div style="font-size:11px;color:var(--muted)"><?= date('l', strtotime($trx['tanggal'])) ?></div>
                            </td>
                            <td>
                                <span style="font-weight:700;color:var(--orange-dark)">Rp <?= number_format($trx['total'], 0, ',', '.') ?></span>
                            </td>
                            <td>
                                <button class="btn btn-blue btn-sm" onclick='lihatDetail(<?= json_encode($trx) ?>)'>
                        ...
                        <tr>
                            <td colspan="5">
                                <div class="empty-state">
                                    ...
                                    <a href="/transaksi/create" class="btn btn-orange" style="margin-top:14px"><i class="fas fa-plus"></i> Buat Transaksi</a>
                                </div>
                            </td>
                        </tr>
                        <?php endif; ?>
    ...
    <div class="modal">
        ...
        <div class="modal-body">
            <div class="detail-header">
                <div>
                    <div class="detail-info-label">Kode Transaksi</div>
                    <div class="detail-info-val" id="dKode">—</div>
                </div>
                <div>
                    <div class="detail-info-label">Tanggal</div>
                    <div class="detail-info-val" id="dTanggal">—</div>
                </div>
            </div>

            <!-- 🔥 Detail item per produk — nanti dari $detail_transaksi saat model sudah dihubungkan -->
            <!-- Struktur: id_detail, id_transaksi, id_produk, qty, harga, subtotal -->
            <div style="color:var(--muted);font-size:12px;margin-bottom:12px;display:flex;align-items:center;gap:6px;">
                ...
                Detail item per produk akan tampil setelah model & relasi terhubung
            </div>
            ...
            <table style="font-size:12.5px;">
                ...
                <tbody id="dItems">
                    <tr><td colspan="4" style="text-align:center;padding:20px;color:var(--muted)">
                        <i class="fas fa-box" style="display:block;font-size:24px;color:var(--blue-dim);margin-bottom:6px"></i>
                        Menunggu koneksi ke detail_transaksi
                    </td></tr>
                </tbody>
            </table>
            ...
            <div class="detail-total-bar">
                ...
                <span style="font-weight:800;font-size:17px;color:var(--orange-dark)" id="dTotal">—</span>
            </div>
        </div>
...
<script>
    ...
    function filterTrx() {
        ...
        document.querySelectorAll('#trxTable tbody tr[data-kode]').forEach(r => {
            r.style.display = r.dataset.kode.includes(q) ? '' : 'none';
        });
    }

    // ── MODAL DETAIL ──
    function lihatDetail(trx) {
        document.getElementById('dKode').textContent    = trx.kode;
        document.getElementById('dTanggal').textContent = trx.tanggal;
        document.getElementById('dTotal').textContent   = 'Rp ' + parseInt(trx.total).toLocaleString('id-ID');
