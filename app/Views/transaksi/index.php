...
<head>
    ...
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .search-wrap i.search-icon { position: absolute; left: 11px; top: 50%; transform: translateY(-50%); color: var(--muted); font-size: 12px; pointer-events: none; }
        .search-wrap input[type="text"] { padding-left: 34px; width: 200px; }
        .search-wrap input[type="date"] { width: 160px; }
        .form-control { padding: 9px 12px; border: 1.5px solid var(--border); border-radius: var(--radius-sm); font-family: inherit; font-size: 13px; color: var(--text); background: var(--surface); outline: none; transition: border .15s; }
        ...
        @media (max-width: 600px) { .content { padding: 16px; } .topbar { padding: 0 16px; } .topbar-time { display: none; } .summary-grid { grid-template-columns: 1fr; } .search-wrap input[type="text"] { width: 130px; } .search-wrap input[type="date"] { width: 130px; } }
    </style>
</head>
<body>
...
<aside class="sidebar" id="sidebar">
    <div class="sidebar-logo">
        <div class="logo-mark" style="background:none;padding:0;">
            <img src="/img/logo.png" alt="Logo" style="width:36px;height:36px;object-fit:contain;">
        </div>
        <div>
...
<div class="main">
    ...
    <div class="content">
        ...
        <!-- TOOLBAR: pencarian teks + filter tanggal -->
        <div class="toolbar">
            <div class="search-wrap">
                <i class="fas fa-search search-icon"></i>
                <input type="text" class="form-control" id="searchInput"
                       placeholder="Cari ID transaksi..." oninput="filterTrx()">
                <input type="date" class="form-control" id="dateInput"
                       title="Filter berdasarkan tanggal" onchange="filterTrx()">
        ...
        <div class="card">
            <div class="card-head">
                ...
                <span class="badge b-blue" id="countBadge"><?= count($transaksi) ?> data</span>
            </div>
            <div class="tw">
                <table id="trxTable">
                    <thead>
                    ...
                    <tbody>
                        ...
                        <!-- data-date pakai format Y-m-d agar mudah dicocokkan input[type=date] -->
                        <tr data-id="<?= esc($trx['id_transaksi']) ?>"
                            data-date="<?= date('Y-m-d', strtotime($trx['tanggal'])) ?>">
                            <td style="color:var(--muted);font-size:12px" class="row-no"><?= $no++ ?></td>
...
<script>
    ...
    // FILTER: ID teks + tanggal
    function filterTrx() {
        const q    = document.getElementById('searchInput').value.toLowerCase().trim();
        const date = document.getElementById('dateInput').value; // format Y-m-d atau kosong
        const rows = document.querySelectorAll('#trxTable tbody tr[data-id]');
        let visible = 0;

        rows.forEach(r => {
            const matchId   = !q    || r.dataset.id.toLowerCase().includes(q);
            const matchDate = !date || r.dataset.date === date;
            const show      = matchId && matchDate;
        ...
        // Update nomor urut
        let no = 1;
        rows.forEach(r => { if (r.style.display !== 'none') r.querySelector('.row-no').textContent = no++; });

        // Update badge jumlah
        document.getElementById('countBadge').textContent = visible + ' data';
        ...
        if (visible === 0) {
            if (!noResult) {
                ...
                noResult.innerHTML = `<td colspan="5"><div class="empty-state"><i class="fas fa-search"></i><p>Tidak ada transaksi yang cocok</p></div></td>`;
                document.querySelector('#trxTable tbody').appendChild(noResult);
            }
        } else {
